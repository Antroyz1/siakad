									...
									<div class="input-group">
										...
										<select name="Kode_Kelas_Jadwal" class="form-control">
											<?php
												...
												$queryjdwjrs = mysqli_query($konek, "SELECT Kode_Kelas_Jadwal, Kode_Kelas, Nama_Kelas FROM jadwal INNER JOIN kelas ON Kode_Kelas_Jadwal=Kode_Kelas WHERE Id_Jadwal='$Id_Jadwal'");
												if($queryjdwjrs == false){
													die ("Terdapat Kesalahan : ". mysqli_error($konek));
												}
												while($jdwjrs = mysqli_fetch_array($queryjdwjrs))
												{
													echo "<option value='$jdwjrs[Kode_Kelas_Jadwal]' selected>$jdwjrs[Nama_Kelas]</option>";
												}
